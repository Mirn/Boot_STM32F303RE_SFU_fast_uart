 ...
 * sfu_commands.c
 *
 *  Created on: 08 �ｿｽ�ｿｽ�ｿｽ�ｿｽ 2016 �ｿｽ.
 *      Author: Easy
 */
...
HAL_StatusTypeDef flash_block_write(uint32_t wr_addr, const uint32_t *dword_ptr, uint32_t dword_count);
...
static void sfu_command_erase(uint8_t code, uint8_t *body, uint32_t size)
{
	...
	if (firmware_size > 0)
	{
		...
		HAL_FLASH_Unlock();
		uint32_t pos = 0;
		uint32_t addr = MAIN_START_FROM;
		HAL_StatusTypeDef status = HAL_ERROR;

		while (addr < (MAIN_START_FROM + (FLASH_SIZE_CORRECT*1024)))
		{
			//printf("Erase page #%i\r", pos);
			...
			{
				uint32_t p = pos / 8;
				packet_send(SFU_CMD_ERASE_PART, (uint8_t *)&p, sizeof(p));
			}
...
#ifdef STM32F303xE
HAL_StatusTypeDef flash_block_write(uint32_t wr_addr, const uint32_t *data, uint32_t count)
{
	//return HAL_OK;
	HAL_StatusTypeDef status = HAL_ERROR;
	HAL_FLASH_Unlock();
